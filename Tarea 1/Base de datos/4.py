    ...
    while len(personas) < cantidad and intentos < max_intentos:
        ...
        if rut not in ruts_usados_globalmente and correo not in correos_usados_globalmente:
            ...
            if len(personas) % 10 == 0:
                 print(f"... {len(personas)}/{cantidad} {tipo_persona} generados.")

    if len(personas) < cantidad:
        print(f"ADVERTENCIA: No se pudieron generar los {cantidad} {tipo_persona} únicos solicitados. Se generaron {len(personas)}. Puede que necesite más intentos o que Faker se haya quedado sin opciones únicas.")
...
def generar_articulos(cantidad, autores_disponibles, topico_a_id_map):
    ...
    for i in range(cantidad):
        ...
        elif "{topico}" in plantilla_titulo:
        ...
        elif "{topico}" in plantilla_resumen:
        ...
        resumen = (resumen_base[:247] + '...') if len(resumen_base) > 250 else resumen_base
        ...
        articulos_generados.append({
            ...
            'resumen': resumen
            # No guardamos autores aquí para mantener la tabla limpia
        })
...
def generar_relaciones_revisor(revisores, articulos, topico_a_id_map, topicos_articulos_relacion):
    ...
    # (Sin cambios respecto a la versión anterior, pero crucial para el siguiente paso)
    ...
    for art in articulos:
        ...
        # --- Lógica Nueva: Encontrar EXACTAMENTE 3 revisores RELEVANTES ---
        revisores_elegibles = []
        ...
        if len(revisores_elegibles) >= REVISORES_POR_ARTICULO:
            # Seleccionar exactamente 3 al azar de los elegibles
...
mapeo_autores = {'rut': 'rut_autor', 'nombre': 'nombre_autor', 'correo': 'correo_autor'}
mapeo_revisores = {'rut': 'rut_revisor', 'nombre': 'nombre_revisor', 'correo': 'correo_revisor'}
...
archivos_a_generar = [
    {'nombre': "autores.csv", 'datos': autores_csv_list, 'encabezados': ['rut_autor', 'nombre_autor', 'correo_autor']},
    {'nombre': "revisores.csv", 'datos': revisores_csv_list, 'encabezados': ['rut_revisor', 'nombre_revisor', 'correo_revisor']},
    {'nombre': "articulos.csv", 'datos': articulos_data, 'encabezados': ['id_articulo', 'titulo', 'fecha_envio', 'resumen']},
    ...
    {'nombre': "revision.csv", 'datos': revision_data, 'encabezados': ['rut_revisor', 'id_articulo']},
    {'nombre': "envio_articulo.csv", 'datos': envio_articulo_data, 'encabezados': ['id_articulo', 'rut_autor', 'correo_contacto', 'userid_contacto', 'password_contacto']}
]
...
print("- IDs secuenciales de 5 dígitos.")